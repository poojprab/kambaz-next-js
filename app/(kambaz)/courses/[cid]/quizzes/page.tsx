...
import { FaPlus, FaSearch, FaEllipsisV, FaCheckCircle, FaBan } from "react-icons/fa";
...
export default function Quizzes() {
  ...
  return (
    <div id="wd-quizzes" className="p-3">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div className="input-group w-50">
          ...
          <input type="text" className="form-control"
            placeholder="Search for Quiz" id="wd-search-quiz" />
        </div>
        {isFaculty && (
          <button className="btn btn-danger" id="wd-add-quiz"
      ...
      <ul id="wd-quiz-list" className="list-group list-group-flush border border-top-0">
          ...
          <li key={quiz._id} className="list-group-item"
            ...
            <div className="d-flex justify-content-between align-items-center">
              <div className="d-flex align-items-center">
                ...
                <div>
                  ...
                  <div className="small text-muted">
                    <span className={
                      getAvailabilityStatus(quiz) === "Available" ? "text-success" :
                      getAvailabilityStatus(quiz) === "Closed" ? "text-danger" : ""
                    }>
                      {getAvailabilityStatus(quiz)}
                    </span>
                    {" | "}
                    <b>Due</b> {new Date(quiz.dueDate).toLocaleDateString()} | {quiz.points} pts | {quiz.questions?.length || 0} Questions
                  </div>
                </div>
              </div>
              ...
              <div className="d-flex align-items-center">
                {isFaculty && (
                  <>
                    <span onClick={() => dispatch(togglePublish(quiz._id))}
                      style={{ cursor: "pointer" }} className="me-2">
                      {quiz.published
